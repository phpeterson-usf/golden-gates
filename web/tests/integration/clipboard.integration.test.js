...
describe('Clipboard Integration Workflows', () => {
  ...
  beforeEach(() => {
    ...
    circuitModel = {
      ...
      removeComponent: vi.fn(componentId => {
        const index = circuitModel.activeCircuit.value.components.findIndex(
          c => c.id === componentId
        )
        if (index >= 0) {
  ...
  describe('Duplicate Workflow', () => {
    ...
    it('should handle duplicate with wire connections', () => {
      ...
      expect(duplicatedElements.wires[0].startConnection.componentId).toBe(
        duplicatedElements.components[0].id
      )
      expect(duplicatedElements.wires[0].endConnection.componentId).toBe(
        duplicatedElements.components[1].id
      )
    })
  })
  ...
  describe('Performance Workflows', () => {
    it('should handle large clipboard operations', () => {
      ...
      const largeElements = {
        ...
        wires: Array.from({ length: 50 }, (_, i) => testUtils.createMockWire({ id: `wire_${i}` })),
