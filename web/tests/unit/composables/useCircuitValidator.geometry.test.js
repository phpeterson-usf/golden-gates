...
describe('useCircuitValidator - Geometry-based connections', () => {
  ...
  it('should handle duplicated components and wires based on geometry', () => {
    ...
    const duplicatedInput3ToAnd = connections.find(conn => 
      conn.sourceComponent.id === 'input3' && conn.targetComponent.id === 'and2'
    )
    expect(duplicatedInput3ToAnd).toBeDefined()

    const duplicatedInput4ToAnd = connections.find(conn => 
      conn.sourceComponent.id === 'input4' && conn.targetComponent.id === 'and2'
    )
    expect(duplicatedInput4ToAnd).toBeDefined()

    const duplicatedAndToOutput = connections.find(conn => 
      conn.sourceComponent.id === 'and2' && conn.targetComponent.id === 'output2'
    )
